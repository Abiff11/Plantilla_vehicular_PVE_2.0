...
import { VehicleImportBatchEntity, VehicleImportBatchStatus } from '../entities/vehicle-import-batch.entity';
...
export class RecordsImportService {
  constructor(
    @InjectRepository(RecordEntity) private readonly recordRepository: Repository<RecordEntity>,
    @InjectRepository(UserEntity) private readonly userRepository: Repository<UserEntity>,
    @InjectRepository(DelegationEntity) private readonly delegationRepository: Repository<DelegationEntity>,
    @InjectRepository(CatalogGroupEntity) private readonly catalogGroupRepository: Repository<CatalogGroupEntity>,
    @InjectRepository(CatalogItemEntity) private readonly catalogItemRepository: Repository<CatalogItemEntity>,
    @InjectRepository(CatalogAliasEntity) private readonly catalogAliasRepository: Repository<CatalogAliasEntity>,
    @InjectRepository(VehicleImportBatchEntity) private readonly importBatchRepository: Repository<VehicleImportBatchEntity>,
    @InjectRepository(VehicleImportErrorEntity) private readonly importErrorRepository: Repository<VehicleImportErrorEntity>,
  ...
  async preview(file: UploadedExcelFile, authUser: AuthUser) {
    ...
    await this.auditLogsService.register({ actorId: authUser.sub, action: 'VEHICLE_IMPORT_PREVIEWED', entityType: 'vehicle_import_batch', entityId: batch.id, metadata: { fileName: file.originalname, sheetName: parsed.sheetName, totalRows: summary.totalRows, validRows: summary.validRows, invalidRows: summary.invalidRows } });
    return { importBatchId: batch.id, ...summary, sampleRows: rows.slice(0, 20).map((row) => row.normalized) };
  }
  ...
  async findImportErrors(batchId: string) {
    ...
    if (!batch) throw new NotFoundException('No se encontro el lote de importacion.');
    return this.importErrorRepository.find({ where: { batch: { id: batchId } }, order: { rowNumber: 'ASC', createdAt: 'ASC' } });
  }

  private async resolveUser(authUser: AuthUser) {
    const user = await this.userRepository.findOneBy({ id: authUser.sub });
    if (!user) throw new NotFoundException('No se encontro el usuario autenticado.');
    return user;
  }

  private async resolveDefaultDelegation(authUser: AuthUser) {
    if (authUser.delegationId) {
      const assignedDelegation = await this.delegationRepository.findOne({ where: { id: authUser.delegationId }, relations: { region: true } });
      if (assignedDelegation) return assignedDelegation;
    }
  ...
  private async createBatch(summary: ReturnType<RecordsImportService['buildImportSummary']>, user: UserEntity, status: VehicleImportBatchStatus, importedRows: number) {
    return this.importBatchRepository.save(this.importBatchRepository.create({ fileName: summary.fileName, sheetName: summary.sheetName, totalRows: summary.totalRows, validRows: summary.validRows, invalidRows: summary.invalidRows, importedRows, status, sourceSections: summary.sourceSections, pendingCatalogValues: summary.pendingCatalogValues, finishedAt: status === VehicleImportBatchStatus.Imported || status === VehicleImportBatchStatus.Failed ? new Date() : null, createdBy: user }));
  }

  private async persistImportErrors(batch: VehicleImportBatchEntity, rows: ImportRow[]) {
    const errors = rows.flatMap((row) => row.errors.map((message) => this.importErrorRepository.create({ batch, rowNumber: row.sourceRowNumber, section: row.sourceSection, columnName: resolveErrorColumnName(message), rawValue: resolveErrorRawValue(message, row), errorType: resolveErrorType(message), message })));
    return errors.length === 0 ? [] : this.importErrorRepository.save(errors, { chunk: 100 });
  }

  private buildImportSummary(fileName: string, sheetName: string, rows: ImportRow[], catalogLookup: CatalogLookup) {
    const invalidRows = rows.filter((row) => row.errors.length > 0);
    const pendingCatalogValues = this.findPendingCatalogValues(rows, catalogLookup);
    return { fileName, sheetName, totalRows: rows.length, validRows: rows.length - invalidRows.length, invalidRows: invalidRows.length, sourceSections: Array.from(new Set(rows.map((row) => row.sourceSection).filter(Boolean))), pendingCatalogValues, errors: invalidRows.map((row) => ({ rowNumber: row.sourceRowNumber, section: row.sourceSection, messages: row.errors })) };
  }
  ...
  private assertImportRole(authUser: AuthUser) {
    if ([Role.PlantillaVehicular, Role.SuperAdmin, Role.Coordinacion].includes(authUser.role)) return;
  ...
  private async parseImportRows(file: UploadedExcelFile) {
    ...
    const sheet = workbook.sheets.find((candidate) => candidate.name === 'PLANTILLA GENERAL 2025 (IMPRIM)') ?? workbook.sheets[0];
    if (!sheet) throw new BadRequestException('El Excel no contiene hojas legibles.');
    const headerIndex = sheet.rows.findIndex((row) => row.some((cell) => normalizeHeader(cell) === 'CIV') && row.some((cell) => normalizeHeader(cell) === 'PLACAS 2026'));
    if (headerIndex < 0) throw new BadRequestException('No se encontro la fila de encabezados esperada.');
    const headers = sheet.rows[headerIndex].map(normalizeHeader);
    const missingHeaders = EXPECTED_HEADERS.filter((expectedHeader) => !headers.includes(normalizeHeader(expectedHeader)));
    if (missingHeaders.length > 0) throw new BadRequestException(`Faltan columnas obligatorias en el Excel: ${missingHeaders.join(', ')}.`);
    ...
    for (let index = headerIndex + 1; index < sheet.rows.length; index += 1) {
      ...
      if (!isVehicleRow(values)) continue;
      const normalized = normalizeExcelImportRecord(values, currentSection, index + 1);
      if (!isImportableVehicleRow(normalized)) continue;
      rows.push({ sourceRowNumber: index + 1, sourceSection: currentSection, values, normalized, errors: [] });
    }
  ...
  private async buildCatalogLookup(): Promise<CatalogLookup> {
    const groups = await this.catalogGroupRepository.find({ where: { code: In(CATALOG_FIELD_MAP.map((entry) => entry.catalogCode)) }, relations: { items: true } });
    const aliases = await this.catalogAliasRepository.find({ relations: { catalogItem: { group: true } } });
  ...
  private async validateRows(rows: ImportRow[], catalogLookup: CatalogLookup) {
    ...
    for (const row of rows) row.errors = validateRow(row.normalized, catalogLookup, duplicateLookup);
    return rows;
  }

  private async buildDuplicateLookup(rows: ImportRow[]): Promise<DuplicateLookup> {
    const plates = rows.map((row) => row.normalized.plates).filter((value) => value && isStrongPlateValue(value));
    const serialNumbers = rows.map((row) => row.normalized.serialNumber).filter((value) => value && !isGenericIdentifierValue(value));
    const engineNumbers = rows.map((row) => row.normalized.engineNumber).filter((value) => value && !isGenericIdentifierValue(value));
    const civs = rows.map((row) => row.normalized.civ).filter((value) => value && isStrongCivValue(value));
    const where = [...(plates.length ? [{ plates: In(plates) }] : []), ...(serialNumbers.length ? [{ serialNumber: In(serialNumbers) }] : []), ...(engineNumbers.length ? [{ engineNumber: In(engineNumbers) }] : []), ...(civs.length ? [{ civ: In(civs) }] : [])];
    const existingRecords = where.length > 0 ? await this.recordRepository.find({ where }) : [];
    return { plates: countValues(plates), serialNumbers: countValues(serialNumbers), engineNumbers: countValues(engineNumbers), civs: countValues(civs), existingPlates: new Set(existingRecords.map((record) => record.plates).filter(isStrongPlateValue)), existingSerialNumbers: new Set(existingRecords.map((record) => record.serialNumber).filter((value) => Boolean(value) && !isGenericIdentifierValue(value))), existingEngineNumbers: new Set(existingRecords.map((record) => record.engineNumber).filter((value) => Boolean(value) && !isGenericIdentifierValue(value))), existingCivs: new Set(existingRecords.map((record) => record.civ).filter(isStrongCivValue)) };
  }
...
function validateRow(record: NormalizedExcelImportRecord, catalogLookup: CatalogLookup, duplicateLookup: DuplicateLookup) {
  const errors: string[] = [];
  if (!record.plates && !record.serialNumber && !record.civ) errors.push('La fila requiere placas, numero de serie o CIV para identificar la unidad.');
  for (const requiredField of ['type', 'useType', 'vehicleClass', 'model', 'serialNumber', 'custodian', 'physicalStatus', 'status'] as const) if (!record[requiredField]) errors.push(`Campo obligatorio vacio: ${requiredField}.`);
  for (const entry of CATALOG_FIELD_MAP) {
    const value = record[entry.field];
    if (!value) continue;
    const catalogValues = catalogLookup.get(entry.catalogCode) ?? new Set<string>();
    if (entry.required && !catalogValues.has(normalizeCatalogValue(String(value)))) errors.push(`Valor pendiente de catalogo ${entry.catalogCode}: ${value}.`);
  }
  if (record.plates && isStrongPlateValue(record.plates) && (duplicateLookup.plates.get(record.plates) ?? 0) > 1) errors.push(`Placas duplicadas dentro del Excel: ${record.plates}.`);
  if (record.serialNumber && !isGenericIdentifierValue(record.serialNumber) && (duplicateLookup.serialNumbers.get(record.serialNumber) ?? 0) > 1) errors.push(`Numero de serie duplicado dentro del Excel: ${record.serialNumber}.`);
  if (record.civ && isStrongCivValue(record.civ) && (duplicateLookup.civs.get(record.civ) ?? 0) > 1) errors.push(`CIV duplicado dentro del Excel: ${record.civ}.`);
  if (record.engineNumber && !isGenericIdentifierValue(record.engineNumber) && (duplicateLookup.engineNumbers.get(record.engineNumber) ?? 0) > 1) errors.push(`Numero de motor duplicado dentro del Excel: ${record.engineNumber}.`);
  if (record.plates && isStrongPlateValue(record.plates) && duplicateLookup.existingPlates.has(record.plates)) errors.push(`Las placas ya existen en una captura activa: ${record.plates}.`);
  if (record.serialNumber && !isGenericIdentifierValue(record.serialNumber) && duplicateLookup.existingSerialNumbers.has(record.serialNumber)) errors.push(`El numero de serie ya existe en una captura activa: ${record.serialNumber}.`);
  if (record.engineNumber && !isGenericIdentifierValue(record.engineNumber) && duplicateLookup.existingEngineNumbers.has(record.engineNumber)) errors.push(`El numero de motor ya existe en una captura activa: ${record.engineNumber}.`);
  if (record.civ && isStrongCivValue(record.civ) && duplicateLookup.existingCivs.has(record.civ)) errors.push(`El CIV ya existe en una captura activa: ${record.civ}.`);
  return errors;
}
